...
# QuantileRegression_solver() : Solve given dantzig selector problem    #
...
Dantzig_solver <- function(X, y, max_it = 50, lambda_threshold = 0.01){
  ...
  d <- ncol(X)
  t <- 0
  lambdalist <- rep(0.0, max_it)
  x_list <- matrix(0.0, d, max_it)
  y_list <- rep(0.0, max_it)
  str <- .C("Dantzig_api", as.integer(n), as.integer(d),
          as.double(t(X)), as.double(y),
