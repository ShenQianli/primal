...
from .core import _load_lib, PSMmodel

__all__ = ["QuantileRegression", "SparseSVM", "Dantzig", "CompressedSensing"]

_PSM_LIB = _load_lib()

class QuantileRegression(PSMmodel):
    ...
    def _decor_cinterface(self, _function, max_it, lambda_threshold):
        ...
        def wrapper():
            ...
            _function(
                ...
                self.X, 
                self.y, 
                np.array([self.tau], dtype='double'), 
...
class SparseSVM(PSMmodel):
    ...
    def _decor_cinterface(self, _function, max_it, lambda_threshold):
        ...
        def wrapper():
            time_start = time.time()
            _function(np.array([self.n], dtype='int32'), 
                np.array([self.d], dtype='int32'), 
                self.X, 
                self.y,
                np.array([max_it], dtype='int32'), 
...
class CompressedSensing(PSMmodel):
